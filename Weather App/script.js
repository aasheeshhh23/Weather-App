...
const API_KEY= "b3469def46834b449ff83054261001";

// weather code for maping their corresponding icons
const weatherCodes = {
    clear : [1000],
...
try {
    // Fetch data from Api and parse the response as json format
    ...
    const weatherIcon = Object.keys(weatherCodes).find(icon => weatherCodes[icon].includes(data.current.
    condition.code))
...
// combine hourly data from today and tomorrow
    const combinedHourlyData = [...data.forecast.forecastday[0].hour,...data.forecast.forecastday[1].
    hour]
